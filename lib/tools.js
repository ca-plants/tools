...
class Tools {
    ...
    static getCommandLineOptions( optionDefs, parseOptions = {} ) {
        return commandLineArgs( optionDefs.concat( OPTION_DEFS ), parseOptions );
    }
    ...
    static async run( DataLoader ) {
        ...
        const taxa = await DataLoader.load( options );
        ...
        for ( const tool of tools ) {
            switch ( tool ) {
                ...
                case TOOLS.JEPSON_EFLORA:
                    await JepsonEFlora.analyze( TOOLS_DATA_DIR, taxa, exceptions, options[ "ef-lognotes" ] );
                    break;
                case TOOLS.JEPSON_FAM:
                    await JepsonFamilies.build( TOOLS_DATA_DIR );
                    break;
                case TOOLS.RPI:
                    await RPI.analyze( TOOLS_DATA_DIR, taxa, exceptions );
                    break;
                case TOOLS.TEXT:
