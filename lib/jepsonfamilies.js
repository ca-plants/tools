...
class JepsonFamilies {
    ...
    static async build( toolsDataDir ) {
        ...
        const document = parse( Files.read( indexFilePath ) );
        const body = HTMLTree.getSubTree( document, t => t.tagName === "body" );
        const contentDiv = HTMLTree.getSubTree( body, t => HTMLTree.getAttr( t, "id" ) === "content" );
        const rows = HTMLTree.getSubTrees( contentDiv, t => t.tagName === "tr" );

        this.#parseRows( toolsDataPath, rows );
    }
    ...
    static #parseRows( toolsDataPath, rows ) {
        ...
        for ( const row of rows ) {
            ...
            // Find all the genera.
            const genusLinks = HTMLTree.getSubTrees( cols[ 2 ], t => t.tagName === "a" );
        ...
        Files.write( toolsDataPath + "/families.json", JSON.stringify( families ), true );
        Files.write( toolsDataPath + "/genera.json", JSON.stringify( genera ), true );
