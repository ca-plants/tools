...
class JepsonInterchange {
    ...
    static async analyze( toolsDataDir, Taxa ) {
        ...
        async function loadNameIndex( firstLetter ) {
            ...
            for ( const match of matches ) {
                ...
                JepsonInterchange.#nameInfo[ name ] = { id: id, category: category };
            }

            JepsonInterchange.#loadedLetters[ firstLetter ] = true;
        }

        async function getJepsInfo( name ) {
            const firstLetter = name[ 0 ];
            // See if this index has been loaded.
            if ( !JepsonInterchange.#loadedLetters[ firstLetter ] ) {
                await loadNameIndex( firstLetter );
            }

            return JepsonInterchange.#getJepsInfo( name );
        }

        const toolsDataPath = toolsDataDir + "/jepson-int";
        ...
        for ( const taxon of Taxa.getTaxa() ) {
            ...
            if ( jepsInfo === undefined ) {
                ...
                if ( !Exceptions.hasException( name, "jepson", "notinnameindex" ) ) {
                    LogMessage.log( name, "not found in Jepson Interchange index" );
                }
                continue;
            }
            ...
            if ( taxon.getJepsonID() !== jepsInfo.id ) {
                if ( !Exceptions.hasException( name, "jepson", "badjepsonid" ) ) {
                    LogMessage.log( name, "Jepson ID does not match ID from Jepson Interchange index", taxon.getJepsonID(), jepsInfo.id );
                }
            }
            ...
            switch ( jepsInfo.category ) {
                ...
                case "JFP-10":
                    ...
                    if ( !taxon.isCANative() ) {
                        LogMessage.log( name, "Jepson Interchange index has category JFP-1 but taxa.csv says not native" );
                    }
                    break;
                case "JFP-1a":
                    if ( !Exceptions.hasException( name, "jepson", "allowsynonym" ) ) {
                        LogMessage.log( name, "is synonym in Jepson Interchange index" );
                    }
                    break;
                case "JFP-1b":
                    LogMessage.log( name, "is JFP-1b in Jepson Interchange index - invalid name" );
                    break;
                case "JFP-2":
                case "JFP-2_t":
                ...
                case "JFP-4":
                    ...
                    if ( taxon.isCANative() ) {
                        LogMessage.log( name, "Jepson Interchange index has category " + jepsInfo.category + " but taxa.csv says native" );
                    }
                    break;
                case "JFP-unresolved":
                    // Ignore for now.
                ...
                case "JFP-11":
                case "JFP-11a":
                    if ( !Exceptions.hasException( name, "jepson", "ignoreunrecognized" ) ) {
                        LogMessage.log( name, "Jepson Interchange index has category " + jepsInfo.category + "; not taxonomically recognized" );
                    }
                    break;
                default:
    ...
    static #getJepsInfo( name ) {
        ...
        name = name.replace( "×", "X " );
        return JepsonInterchange.#nameInfo[ name ];
    }

}

export { JepsonInterchange };