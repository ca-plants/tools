...
class Calflora {
    ...
    static async analyze( toolsDataDir, Taxa, retrieveToolData ) {
        ...
        if ( retrieveToolData ) {
            ...
            await Files.fetch( CALFLORA_URL_ALL + "1", toolsDataDir + "/" + calfloraDataFileNameActive );
            await Files.fetch( CALFLORA_URL_ALL + "0", toolsDataDir + "/" + calfloraDataFileNameInactive );
            // County list and "all" lists are both incomplete; load everything to get as much as possible.
            await Files.fetch( CALFLORA_URL_COUNTY, toolsDataDir + "/" + calfloraDataFileNameCounties );
        }
        ...
        const csvCounties = CSV.parseFile( toolsDataDir, calfloraDataFileNameCounties );
