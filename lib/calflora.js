...
class Calflora {
    ...
    static async analyze( toolsDataDir, taxa, exceptions ) {
        ...
        for ( const taxon of taxa.getTaxa() ) {
            ...
            if ( cfJepsonID !== taxon.getJepsonID() ) {
                if ( !exceptions.hasException( name, "calflora", "badjepsonid" ) && !exceptions.hasException( name, "calflora", "notintaxondata" ) ) {
                    LogMessage.log( name, "Jepson ID in Calflora is different than taxa.csv", cfJepsonID, taxon.getJepsonID() );
                }
            }
            ...
            if ( cfID !== taxon.getCalfloraID() ) {
                LogMessage.log( name, "Calflora ID in Calflora is different than taxa.csv", cfID, taxon.getCalfloraID() );
            }
        }
