    ...
    static async analyze( toolsDataDir, taxa, exceptions, errorLog ) {
        ...
        for ( const taxon of taxa.getTaxonList() ) {
            ...
            if ( cfIsNative !== taxon.isCANative() ) {
                errorLog.log( name, "has different nativity status in Calflora", cfIsNative );
            }
            ...
            if ( cfJepsonID !== taxon.getJepsonID() ) {
                if ( !exceptions.hasException( name, "calflora", "badjepsonid" ) && !exceptions.hasException( name, "calflora", "notintaxondata" ) ) {
                    errorLog.log( name, "Jepson ID in Calflora is different than taxa.csv", cfJepsonID, taxon.getJepsonID() );
                }
            }
    ...
    static #checkExceptions( taxa, exceptions, errorLog ) {
        ...
        for ( const [ name, v ] of exceptions.getExceptions() ) {
            ...
            if ( !taxon ) {
                ...
                errorLog.log( name, "has Calflora exceptions but not in Taxa collection" );
                continue;
            }
            ...
            for ( const [ k ] of Object.entries( exceptions ) ) {
                ...
                switch ( k ) {
                    case "notintaxondata":
                        if ( cfData ) {
                            errorLog.log( name, "found in Calflora data but has notintaxondata exception" );
                        }
                        break;
                    case "badjepsonid": {
                        ...
                        if ( cfID === jepsID ) {
                            errorLog.log( name, "has Calflora badjepsonid exception but IDs are the same" );
                        }
                        break;
                    }
                    default:
                        errorLog.log( name, "unrecognized Calflora exception", k );
                }
            }
        }
    }
