...
class Calflora {
    ...
    static async analyze( toolsDataDir, Taxa ) {
        ...
        async function retrieveCalfloraFile( url, targetFile ) {
            ...
            targetFile = toolsDataDir + "/" + targetFile;
        ...
        await retrieveCalfloraFile( CALFLORA_URL_ALL + "0", calfloraDataFileNameInactive );
    ...
    static #checkExceptions( Taxa ) {
        ...
        for ( const [ name, v ] of Exceptions.getExceptions() ) {
            ...
            if ( !taxon ) {
                LogMessage.log( name, "has Calflora exceptions but not in taxa.tsv" );
                continue;
            }
