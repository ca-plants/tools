    ...
    static async analyze( toolsDataDir, taxa, config, exceptions, errorLog ) {
        ...
        function checkStatusMatch( name, label, rpiRank, taxonRank ) {
            if ( rpiRank !== taxonRank ) {
                errorLog.log( name, label + " rank in taxa.csv is different than rank in " + fileName, taxonRank, rpiRank );
            }
        }
            ...
            if ( shouldBeInGeo ) {
                if ( !taxon ) {
                    if ( cesa && countyCodes !== undefined ) {
                        errorLog.log( name, "is CESA listed but not found in taxa.csv", cesa );
                    }
                    ...
                    if ( countyCodes !== undefined ) {
                        errorLog.log( name, "in RPI but not found in taxa.csv", rank );
                    }
                    continue;
                }
            } else {
                ...
                if ( taxon.isNative() && !this.#hasExceptions( name, exceptions, "ingeo" ) ) {
                    ...
                    errorLog.log( name, "in taxa.csv but not in RPI for local counties", rank );
                }
            }

            if ( rank !== taxon.getRPIRankAndThreat() ) {
                if ( taxon.isNative() ) {
                    errorLog.log( name, "rank in taxa.csv is different than rank in " + fileName, taxon.getRPIRankAndThreat(), rank );
                }
            }
        ...
        for ( const taxon of taxa.getTaxonList() ) {
            ...
            if ( taxon.getRPIRankAndThreat() ) {
                ...
                if ( !this.#rpiData[ translatedName ? translatedName : name ] ) {
                    errorLog.log( name, "has CRPR in taxa.csv but is not in " + fileName );
                }
            } else {
        ...
        for ( const [ name, v ] of exceptions.getExceptions() ) {
            ...
            if ( !rpiData ) {
                ...
                if ( !rpiExceptions[ "translation-to-rpi" ] ) {
                    errorLog.log( name, "has rpi exception but is not in rpi.csv" );
                }
            }
            ...
            for ( const [ k, v ] of Object.entries( rpiExceptions ) ) {
                switch ( k ) {
                    case "extirpated": {
                        ...
                        if ( taxon ) {
                            errorLog.log( name, "has rpi extirpated exception but is in taxa.csv" );
                        }
                        // Make sure it has extirpated RPI status.
                        const rank = rpiData[ "CRPR" ];
                        if ( rank !== "1A" && rank !== "2A" ) {
                            errorLog.log( name, "has rpi extirpated exception rank is not 1A or 2A" );
                    ...
                    case "ingeo":
                        ...
                        if ( !taxon ) {
                            errorLog.log( name, "has rpi ingeo exception but is not in taxa.csv" );
                        }
                        // Make sure it is no listed in local area in RPI.
                        if ( this.#shouldBeHere( rpiData, countyCodes ) ) {
                            errorLog.log( name, "has rpi ingeo exception but is listed in local counties in rpi.csv" );
                        }
                        break;
                    case "non-native":
                        // Make sure the taxon is in our list.
                        if ( !taxon ) {
                            errorLog.log( name, "has rpi non-native exception but is not in taxa.csv" );
                            continue;
                        }
                        // Make sure it is non-native in our list.
                        if ( taxon.isCANative() ) {
                            errorLog.log( name, "has rpi non-native exception but is native in local list" );
                        }
                        break;
                    case "notingeo":
                        // Make sure the taxon is not in our list.
                        if ( taxon ) {
                            errorLog.log( name, "has rpi notingeo exception but is in taxa.csv" );
                        }
                        // Make sure it is listed in local area in RPI.
                        if ( !this.#shouldBeHere( rpiData, countyCodes ) ) {
                            errorLog.log( name, "has rpi notingeo exception but is not listed in local counties in rpi.csv" );
                        }
                        break;
                    case "translation": {
                        // Make sure the translated name is in our list.
                        if ( !taxon ) {
                            errorLog.log( name, "has rpi translation exception, but target not found" );
                        }
                        // Make sure there is a matching translation exception.
                        const translatedName = exceptions.getValue( v, "rpi", "translation-to-rpi" );
                        if ( translatedName !== name ) {
                            errorLog.log( name, "has rpi translation exception, but no reverse translation" );
                        }
                        break;
                    }
                    case "translation-to-rpi": {
                        ...
                        if ( translatedName !== name ) {
                            errorLog.log( name, "has rpi translation-to-rpi exception, but no reverse translation" );
                        }
                        break;
                    }
                    default:
    ...
    static async #scrape( toolsDataDir, taxa, exceptions, errorLog ) {
        ...
        for ( const taxon of taxa.getTaxonList() ) {
            ...
            if ( id !== taxon.getRPIID() ) {
                errorLog.log( name, "RPI ID in " + fileName + " does not match site data", id, taxon.getRPIID() );
            }
        }
    }
