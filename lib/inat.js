...
class INat {
    ...
    static async analyze( toolsDataDir, taxa, exceptions, csvFileName ) {
        ...
        for ( const data of missingTaxa ) {
            await this.#findCurrentName( taxa, exceptions, data.name, data.iNatName );
        }

        this.#checkExceptions( taxa, exceptions );
    }

    static #checkExceptions( taxa, exceptions ) {
    ...
    static async #findCurrentName( taxa, exceptions, name, iNatName ) {
        ...
        if ( result === undefined ) {
            if ( !exceptions.hasException( name, "inat", "notintaxondata" ) ) {
                ...
                if ( iNatID ) {
                    LogMessage.log( name, "iNat lookup failed but has iNat ID", iNatID );
                }
            }
        } else {
