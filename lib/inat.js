...
class INat {
    ...
    static async analyze( toolsDataDir, Taxa, retrieveToolData, csvFileName ) {

        if ( retrieveToolData ) {
            const url = "https://www.inaturalist.org/taxa/inaturalist-taxonomy.dwca.zip";
            const zipFileName = path.basename( url );
            const csvFilePath = toolsDataDir + "/" + csvFileName;
            console.log( "retrieving iNaturalist species" );
            await Files.fetch( url, csvFilePath );
            await Files.zipFileExtract( toolsDataDir + "/" + zipFileName, "taxa.csv", csvFilePath );
        }

        console.log( "loading iNaturalist species" );
        await CSV.parseStream( toolsDataDir, csvFileName, undefined, undefined, this.#checkTaxon );
